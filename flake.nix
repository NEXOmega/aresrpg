    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        nodeSources = pkgs.runCommand "node-sources" { } ''
          tar --no-same-owner --no-same-permissions -xf ${pkgs.nodejs-15_x.src}
          mv node-* $out
        '';
        ...
        aresrpg = pkgs.stdenv.mkDerivation {
          ...
          nativeBuildInputs =
            [ pkgs.nodejs-15_x cairo pango pkgs.python3 pkgs.pkg-config ];
          ...
          installPhase = ''
            ...
            npm ci --production --build-from-source --nodedir=${nodeSources}
      ...
      in {
        ...
        dockerImage = pkgs.dockerTools.buildImage {
          ...
          config = {
            Cmd =
              [ "${pkgs.nodejs-slim-15_x}/bin/node" "${aresrpg}/src/index.js" ];
            ExposedPorts = {
              "25565/tcp" = { };
